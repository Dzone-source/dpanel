...
final class NodeToken implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if ($_ENV['enable_rate_limit'] &&
            ...
            // 429 — not 401 — so XrayR does not treat this as a bad key and wipe users.
        ...
        if ($expectedHost === '' && isset($_ENV['webAPIUrl'])) {
            // Fallback when webAPIUrl has no scheme (legacy "host" only).
        ...
        // Default off in Docker/NAT: XrayR outbound IP often ≠ node.server DNS.
